...
,replace(replace(convert(nvarchar,convert(money,ISNULL(cast(sum(STout.OutPcs) as int),0)),1), '.00',''),',',' ') as TotalDocWaste
,replace(replace(convert(nvarchar,convert(money,ISNULL(cast(sum(ripoi.planned-isnull(st.InHouse,0)-isnull(STout.OutPcs,0)) as int),0)),1), '.00',''),',',' ') as DocPerc
...
WHERE  PCAD1.IsRequestParam = 1 and RAD1.Deleted is null and PCAD1.DataName = 'Ref. numer zamówienia' and RI.Deleted IS NULL -- jak to skasowalem to sie pojawilo 242
and PCAD2.IsRequestParam = 1 and RAD2.Deleted is null  and PCAD2.DataName = 'Nazwa ref. towaru' -- tutaj mi rozczepia 
...
where (ds.IsWZ = '1' or ds.IsRW = '1') and d.realizated is not null and d.Status='3'
...
and ripoi.Param1 ='FT/20-0242/01' or ripoi.Param1 ='FT/20-0241/01'  -- dla porownania z prawidlowym odfiltrowalem sobie
