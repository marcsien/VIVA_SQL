...
/****** Object:  UserDefinedFunction [dbo].[BilansVPPMWysylki]    Script Date: 2021-05-05 10:08:22 ******/
SET ANSI_NULLS ON
GO
...
FROM         dbo.ProductBatch AS PB INNER JOIN
                      dbo.ProductDefinition AS PD ON PB.ProductDefinitionId = PD.Id INNER JOIN
                          (SELECT     ISNULL((CASE WHEN (DS.IsWZ = 1 OR ((DS.NAME='MMZ' or ds.Name='Korekta MMZ') and (DW.WarehouseSrcName<>'Magazyn zewnêtrzny Ejko' and DW.WarehouseSrcName<>'Magazyn zewnêtrzny Technicolor' and DW.WarehouseSrcName<>'Magazyn zewnêtrzny Sklad')))
														THEN SUM(isnull(TILP.Count, 0)) 
												   WHEN  ((ds.Name='MMZ' or ds.Name='Korekta MMZ') and (DW.WarehouseSrcName<>'Magazyn zewnêtrzny Ejko' or DW.WarehouseSrcName='Magazyn zewnêtrzny Technicolor' or DW.WarehouseSrcName='Magazyn zewnêtrzny Sklad') and DW.WarehouseDstName='Wyroby gotowe')
														THEN -SUM(isnull(TILP.Count, 0)) 
												   END), 0) AS COUNT, 
                            ...
                            FROM          dbo.LogicalProduct AS LP WITH (NOLOCK) left JOIN
                                                   ...
                                                   DS.Name = 'RWtace' or DS.Name='RW ZWROTY' or DS.Name='MMZ' or ds.Name='Korekta MMZ')
                            WHERE      (((LP.Deleted > @LRangeDate) AND (LP.Deleted < @URangeDate) 
										and (DS.IsWZ = 1 OR DS.Name = 'RWtace' or DS.Name='RW ZWROTY')
										and ((DW.WarehouseSrcName<>'Magazyn zewnêtrzny Ejko' and DW.WarehouseSrcName<>'Magazyn zewnêtrzny Technicolor' and DW.WarehouseSrcName<>'Magazyn zewnêtrzny Sklad')		   
											 or (ds.Name='MMZ' and (DW.WarehouseSrcName<>'Magazyn zewnêtrzny Ejko' or DW.WarehouseSrcName='Magazyn zewnêtrzny Technicolor' or DW.WarehouseSrcName='Magazyn zewnêtrzny Sklad') and DW.WarehouseDstName='Wyroby gotowe'))
												   ) or ((DS.Name='MMZ' or DS.Name='Korekta MMZ') and DW.DeadlineDate>@MMZLRangeDate AND DW.DeadlineDate<@MMZURangeDate))
                            GROUP BY LP.ProductBatchId, LP.ProductDefinitionId, BR.ExternalSystemId, DS.IsWZ, DS.Name, DW.WarehouseSrcName, DW.WarehouseDstName) AS WYS ON WYS.ProductBatchId = PB.Id AND 
